...
public class NycOpenDataClient {
    ...
    public int streamPages(Consumer<List<NycApiRestaurantDto>> pageConsumer) {
        ...
        while (true) {
            int limit = (maxRecords > 0) ? Math.min(pageSize, maxRecords - total) : pageSize;
            List<NycApiRestaurantDto> page = fetchPage(offset, limit);
            if (page.isEmpty()) break;
            pageConsumer.accept(page);
            total += page.size();
            logger.info("Fetched {} records (total so far: {})", page.size(), total);
            if (page.size() < limit) break;
            if (maxRecords > 0 && total >= maxRecords) break;
            offset += pageSize;
        }

        logger.info("NYC Open Data fetch complete — {} total records", total);
        return total;
    }

    /** @deprecated Use {@link #streamPages(Consumer)} to avoid OOM on large datasets. */
    @Deprecated
    public List<NycApiRestaurantDto> fetchAll() {
        List<NycApiRestaurantDto> all = new ArrayList<>();
        streamPages(all::addAll);
        return all;
    }
