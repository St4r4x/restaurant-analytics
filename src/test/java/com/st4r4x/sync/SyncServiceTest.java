...
class SyncServiceTest {
    ...
    void runSync_returnsSuccessResult_whenApiAndUpsertSucceed() {
        ...
        doAnswer(inv -> {
            Consumer<List<NycApiRestaurantDto>> consumer = inv.getArgument(0);
            consumer.accept(Collections.singletonList(dto));
            return 1;
        }).when(apiClient).streamPages(any());
    ...
    void runSync_handlesCarryOverAcrossPages() {
        ...
        NycApiRestaurantDto p2r2 = row("BBB", "Other Place", "BROOKLYN",  "A", "7",  "2024-01-01T00:00:00.000");

        doAnswer(inv -> {
            Consumer<List<NycApiRestaurantDto>> consumer = inv.getArgument(0);
            consumer.accept(Collections.singletonList(p1r1));
            consumer.accept(Arrays.asList(p2r1, p2r2));
            return 3;
        }).when(apiClient).streamPages(any());
        when(restaurantDAO.upsertRestaurants(anyList())).thenReturn(1);
        ...
        assertTrue(result.isSuccess());
        assertEquals(3, result.getRawRecords());
        verify(restaurantDAO, times(2)).upsertRestaurants(anyList());
    }
