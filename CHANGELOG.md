...
### Security
- All 5 `public` Postgres tables (`users`, `bookmarks`, `password_reset_tokens`, `inspection_reports`, `audit_log`) had no Row-Level Security — flagged by Supabase's security advisor. They're all created by Hibernate's `ddl-auto=update`, which has no notion of RLS, so a fresh/regenerated database would come back up unprotected even after enabling RLS by hand once. Added `schema.sql` (runs after `ddl-auto` via `spring.jpa.defer-datasource-initialization=true` + `spring.sql.init.mode=always`) to re-enable RLS on every startup. No policies needed — the app's own JDBC connection uses Supabase's superuser role and bypasses RLS regardless; nothing in this codebase uses Supabase's anon/authenticated PostgREST roles.
